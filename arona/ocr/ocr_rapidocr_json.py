...
from .vendor.RapidOCR_api import OcrAPI
...
class OCR:
    ...
    @classmethod
    def _get_path(cls):
        path = get_config("user_config.yaml/rapidocr_json/path")
    ...
    def _load_model_if_not_loaded(cls):
        ...
        if cls.ocr_cn is None:
            cls.ocr_cn = OcrAPI(path)
        if cls.ocr_en is None:
            cls.ocr_en = OcrAPI(path, argsStr="--rec=rec_en_PP-OCRv3_infer.onnx")
    ...
    def ocr(cls, mat, mode='cn', det='single_line'):
        ...
        match mode:
            case "digit":
                model: OcrAPI = cls.ocr_en
            case "en":
                model: OcrAPI = cls.ocr_en
            case _:
                model: OcrAPI = cls.ocr_cn

        img_bytes = cls.mat_to_bytes(mat)

        result = model.runBytes(img_bytes)
        if result['code'] > 101:
            raise RuntimeError("RapidOCR runtime error. " + str(result))
